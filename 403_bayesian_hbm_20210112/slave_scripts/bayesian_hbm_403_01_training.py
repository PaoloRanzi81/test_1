...
#     BASE_DIR_INPUT = ('/home/paolo/Dropbox/Univ/scripts/python_scripts/tensorflow/wright_keith/403_bayesian_hbm_20210112')
#     BASE_DIR_OUTPUT = BASE_DIR_INPUT
...
output_file_name_28 = ('output/403_analysis_feature_importance_boxplot.pdf')
...
# Jax - It works with: tpu_driver (it does not work), tpu_driver0.1-dev20200320, tpu_driver_nightly (it works!)
from cloud_tpu_client import Client
c = Client()
c.configure_tpu_version('tpu_driver_nightly', restart_type='ifNeeded') # working with V3 + JAX!
c.wait_for_healthy()


# set TPU driver as backend for JAX
config.FLAGS.jax_xla_backend = "tpu_driver"
# config.FLAGS.jax_backend_target = 'grpc://10.109.84.34' # Not working!
# config.FLAGS.jax_backend_target = 'grpc://10.109.84.34:8466' # Not working!
config.FLAGS.jax_backend_target = 'grpc://10.39.170.194:8470' # TPU V3
print(config.FLAGS.jax_backend_target)

# force GPU usage
numpyro.set_platform(platform='tpu')
...
variant_id_types = pd.Series(pd.unique(user_activities_concat_04.loc[:,'variant_id_original'])).rename('variant_id_types')
...
variant_id_types.sort_values(axis = 0, inplace = True, 
                     ascending = True)
...
# re-set index
variant_id_types.reset_index(drop = True, inplace = True)
...
variant_id_types_string = pd.DataFrame()
...
if (not selected_variables[selected_variables.str.contains('variant_id')].empty):

    # check whether the variable under scrutiny is NOT a hierachical variable
    if (hierarchical_variable != ['variant_id']):
        # for loop for building a list variables' names by combining to strings for 'variant_id'
        for items_07, variant_id_type in enumerate(pd.unique(variant_id_types)):
            
            # initialize Pandas Series + avoid Pandas' warning
            variant_id_tmp = []
            
            # sub-set
            if pd.unique(user_activities_concat_04.loc[:, 'variant_id']).shape[0] >= 2: 
                
                # initialize Pandas Series + avoid Pandas' warning
                variant_id_selected_name = []
                
                # select the variable name
                variant_id_selected_name = selected_variables[selected_variables.str.contains('variant_id')].reset_index(drop = True)
                
                # build Pandas DataFrame
                variant_id_first_level = pd.DataFrame(pd.Series(variant_id_selected_name[0][0:10]), columns = ['first_level_var'])
                
                # build Pandas DataFrame
                variant_id_second_level = pd.DataFrame(pd.Series(variant_id_type), columns = ['second_level_var'])
                               
                # build a meaningful string
                variant_id_feat_tmp = pd.Series(('{}_{}'.format(variant_id_selected_name[0][0:10],
                                                    variant_id_second_level.iloc[0,0]))).astype('string')
                
                # build a meaningful string
                variant_id_tmp = pd.concat([variant_id_feat_tmp,
                                             variant_id_first_level,
                                             variant_id_second_level], 
                                  axis = 1, ignore_index = True, 
                                  sort = False).reset_index(drop = True)
                               
            else:
                
                # if not one-hot encoded leave the variable's name untouched
                variant_id_tmp = pd.DataFrame()
               
            # append
            variant_id_types_string = variant_id_types_string.append(variant_id_tmp, 
                                                   ignore_index = True, sort = False)
    else:
    ...
    if (hierarchical_variable != ['selection_order']):
        ...
        for items_01, selection_order_type in enumerate(pd.unique(selection_order_types)):
            ...
            if pd.unique(user_activities_concat_04.loc[:, 'selection_order']).shape[0] >= 2: 
                ...
                # if not one-hot encoded leave the variable's name untouched
                selection_order_tmp = pd.DataFrame() 
...
###############################################################################
## 5. LOAD TRAINING SET
# # set shared theano variables (with float64 is TPU computing is slower)
# X_continuos_var = X.loc[:, variables_to_be_used[0]]
# X_categorical_selection_var = X.loc[:, variables_to_be_used[1]]
# X_categorical_audience_var = X.loc[:, variables_to_be_used[2]]
# X_categorical_browser_var = X.loc[:, variables_to_be_used[3]]
# X_categorical_city_var = X.loc[:, variables_to_be_used[4]]
# X_categorical_device_var = X.loc[:, variables_to_be_used[5]]


# y_data = y.to_numpy('int8')
# X_continuos = X.loc[:, variables_to_be_used[0]].astype('float32')
...
# set shared theano variables (with float32 is TPU computing is faster) 
X_continuos_var = X.loc[:, variables_to_be_used[0]].astype('float32')
X_categorical_selection_var = X.loc[:, variables_to_be_used[1]].astype('int8')
X_categorical_audience_var = X.loc[:, variables_to_be_used[2]].astype('int8')
X_categorical_browser_var = X.loc[:, variables_to_be_used[3]].astype('int8')
X_categorical_city_var = X.loc[:, variables_to_be_used[4]].astype('int8')
X_categorical_device_var = X.loc[:, variables_to_be_used[5]].astype('int8')


y_data = y.to_numpy('int8')
X_continuos = X.loc[:, variables_to_be_used[0]].astype('float32')
X_categorical_selection = X.loc[:, variables_to_be_used[1]].astype('int8')
X_categorical_audience = X.loc[:, variables_to_be_used[2]].astype('int8')
X_categorical_browser = X.loc[:, variables_to_be_used[3]].astype('int8')
X_categorical_city = X.loc[:, variables_to_be_used[4]].astype('int8')
X_categorical_device = X.loc[:, variables_to_be_used[5]].astype('int8')
...
if run_modality.iloc[0, 0] == 'testing':
    ...
    tune = 1633 # deadline exceeded error
    # tune = 13062 # deadline exceeded error
    # tune = int(round((450000/chains - draws), 0)) # original
...
def model_factory(X_continuos_var, 
                  X_categorical_selection_var, 
                  X_categorical_audience_var,
                  X_categorical_browser_var,
                  X_categorical_city_var,
                  X_categorical_device_var,
                  y_data, 
    ...
    # with pm.Model(coords=coords) as varying_intercept_slope_noncentered:
        ...
        dot_product_continuos = pm.math.dot(theano.tensor.shape_padaxis(X_continuos, axis=1), beta_con_percentage )
...
# run MCMC sampling
with model_factory(X_continuos_var, 
                  X_categorical_selection_var,
                  X_categorical_audience_var,
                  X_categorical_browser_var,
                  X_categorical_city_var,
    ...
    # varying_intercept_slope_noncentered_trace = pm.sampling_jax.sample_tfp_nuts(draws=draws,
    #                                                     tune=tune,
    #                                                     chains=chains,
    #                                                     target_accept=0.95) 
    
    varying_intercept_slope_noncentered_trace = pm.sampling_jax.sample_numpyro_nuts(draws=draws,
                                                        tune=tune,
                                                        chains=chains,
                                                        target_accept=0.95, 
                                                        # progress_bar=False) # worse perfomance (TPU idling more?)
                                                        progress_bar=True) 
    
    # varying_intercept_slope_noncentered_trace = pm.sample(draws=draws,
    #                             tune=tune,
    #                             chains=chains,
    #                             cores=chains,
    #                             target_accept=0.95,
...
# save the trace by ArviZ
arviz_inference = az.convert_to_inference_data(varying_intercept_slope_noncentered_trace)
