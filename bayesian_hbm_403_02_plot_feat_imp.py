...
#     BASE_DIR_INPUT = ('/home/paolo/Dropbox/Univ/scripts/python_scripts/tensorflow/wright_keith/403_bayesian_hbm_20201202')
#     BASE_DIR_OUTPUT = BASE_DIR_INPUT
...
mask_cat_gender = summary_statistics_trace.loc[:, 'priors'].str.contains('beta_cat_tmp_gender')
...
# loop thorough all variables: diagnostics of posterior + MCMC traces (categorical_gender)
for index_15, variab_name_15 in enumerate(pd.unique(X_categorical_gender.sort_values(kind='mergesort'))):

    az.plot_trace(data=arviz_inference,
                        var_names = 'beta_cat_tmp_gender',
                        coords = {'beta_cat_tmp_gender_dim_0': [index_15]},
                      divergences = 'bottom'); 
    plt.ylabel('variables/features',
                fontsize = 8)
    plt.xlabel('posterior + MCMC traces for {}'.format(variab_name_15),
                fontsize = 8)
    
    # save plot as .pdf file
    plt.savefig(os.path.sep.join([BASE_DIR_OUTPUT, ('{}_gender_{}.pdf'.format(
          output_file_name_32,
          variab_name_15))]))
...
mask_cat_gender_arviz = items_list_tmp.loc[:, 'item_name'].str.contains('beta_cat_tmp_gender')
...
mask_gender = feature_importance.loc[:, 'variable'].str.contains('gender')
...
gender_subset = feature_importance.loc[mask_gender, :].reset_index(drop = True)
            ...
            if selection_order_subset_tmp.loc[items_15, 'coef_abs_value'] > 0.2:
                ...
                if selection_order_positive_coeff.to_numpy() > 0.0: 
                   ...
                   if selection_order_left_whisker.to_numpy() > 0.0: 
                      ...
                      #building result's interpretation string
                      selection_order_tmp = pd.Series(('02. {} , GOOD: video {} has a significant impact on such a variable rather than control video!'.format(
...
if (not gender_subset.empty):
    ...
    for items_17, video_type_17 in enumerate(pd.unique(gender_subset.loc[:, 'variable'])):
        ...
        for items_18, target_var_18 in enumerate(gender_subset_tmp.loc[:, 'target_variable']):
            ...
            # build a meaningful string for each significant video (control included)
            if gender_subset_tmp.loc[items_18, 'coef_abs_value'] > 0.2:
                                             
                #sub-set
                mask_gender_actual_tmp = trace_slopes_long.loc[:, 'variable'].eq(video_type_17)
                gender_actual_subset_tmp = trace_slopes_long.loc[mask_gender_actual_tmp, :].reset_index(drop = True)
                mask_gender_actual_subset_tmp = trace_slopes_long.loc[:, 'target_variable'].eq(target_var_18)
                gender_actual_subset_variant_tmp = gender_actual_subset_tmp.loc[mask_gender_actual_subset_tmp, :].reset_index(drop = True)
                
                # select positive (i.e. earning money) coefficient only
                gender_positive_coeff = gender_actual_subset_variant_tmp.loc[gender_actual_subset_variant_tmp.loc[:, 'quantiles'].eq('hdi_50%'), 'actual_coefficient']
                
                # check whether the coefficient is either negative or positive
                if gender_positive_coeff.to_numpy() > 0.0: 
                   
                   # select left-most whisker only 
                   gender_left_whisker = gender_actual_subset_variant_tmp.loc[gender_actual_subset_variant_tmp.loc[:, 'quantiles'].eq('hdi_3%'), 'actual_coefficient']
                   
                   # check whether the left-most does NOT overlap with zero
                   if gender_left_whisker.to_numpy() > 0.0: 
                      
                      #building result's interpretation string
                      gender_tmp = pd.Series(('03. {} , GOOD: video {} has a significant impact on such a variable rather than control video!'.format(
            ...
            # append
            gender_string = gender_string.append(selection_order_tmp, 
                                           ignore_index = True, sort = False)
else: 
    pass
...
# MarkDown: add new header
mdFile.new_header(level=3, 
                  title="Legend explaining how to interpret beta_tmp[ , ] nomenclature")
...
# MarkDown: add new header
mdFile.new_header(level=3, 
                  title="Hierarchical and not-hierarchical variables for the bayesian model")
